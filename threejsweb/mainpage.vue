<template>
    <ef-main :isMainPage="true">
        <div class="top" :style="{height: (pageMinHeight-90)+'px'}">
            <!-- class="fullscreen" -->
            <div class="left" style="background-color:rgb(83, 132, 175);width: 10%;overflow-y: scroll;">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            handle_chosecurbackground(e) {
                this.curbackground = e;
                this.currdiv = this.dd[e].key
                this.currdivobj = this.dd[e]
            },
                    ...
                    if (actions && actions.length > 0) {
                        actions.map(a => {
                            if (a.actiontype == 'click_block') {
                                ...
                                if (that.checkFull()) {
                                    let temp_style = eval("(" + a.actionstyle + ")")
                                    that.handle_chosecurbackground(temp_style.index)
                                }
                            }
                        })
                    }
