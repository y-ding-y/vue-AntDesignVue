<template>
    <ef-main :id="imageId" :isMainPage="true" :title="$t('repairTitle')" :loading="loading" :openSubmit="true"
        ...
        <ef-chart class="basebg" :height="chartHeight" :padding="[24, 64, 48, 64]" :scale="[
          ...
          {
            dataKey: 'key',
            type: ['month', 'week', 'date', 'day', 'hour', 'iodays'].includes(
              groupKey
            )
              ? 'cat'
              : 'linear',
        ...
        <a-table class="basebg" :id="tableId" :size="sSize" :bordered="true" :columns="columns" :pagination="false"
            ...
            <!--表頭title-->
            <span v-for="key in ['sn', 'qty', 'rate', 'accQty', 'accRate']" class="title" :slot="key + 'T'"
                :key="key">{{
                $t(key) }}</span>
        ...
        <!--defect modal-->
        <defect-list :vkey="key" :paras="paras" :prop="'repair'" :title="title" :action-fix="'sfcsSMTRepair'">
        </defect-list>
    </ef-main>
</template>
...
<script>
    ...
    const groupOption = 'line,routeCode,defectType,defectCode,dutyCode,repairerName,iodays,class,family,model,pn,wo,month,week,date,hour'.split(
        ','
    )
    const groupExt = {
        ...
        rootCode: { col: 'rootName' },
        dutyCode: { col: 'dutyName' },
    ...
    const formItems = [
     ...
     //   { key: 'defectType', type: 'select', paraKey: 'SfcsSmt.DEFECTTYPE' },
        // {
        //     key: 'defectCode',
        //     type: 'select',
        //     mode: 'multiple',
        //     paraKey: 'SfcsSmt.DEFECTCODE',
        // },
        // {
        //     key: 'dutyCode',
        //     type: 'select',
        //     mode: 'multiple',
        //     paraKey: 'SfcsSmt.DUTYCODE',
        // },
    ...
    export default {
        ...
        methods: {
            submit(values) {
                const io = values.dateType.left(1)
                values.prop = 'repair'
                const group = { key: values.group }
                this.groupKey = group.key
                if (['month', 'week', 'date', 'day'].includes(values.group)) {
                    values.group = (io + '_' + values.group).toHump()
                }
                values[io + 'Time'] = values.Time
                values.class = values.class ? values.class : ['MB']
                delete values.Time
