			...
			<a-tab-pane v-for="(item, index) in snlist" :key="index" :tab="item">
				<a-spin :spinning="listLoading">
					<a-collapse v-model="activeKey">
						<a-collapse-panel v-for="item in collapselist" :key="item.key" :header="item.name"
							...
							<v-form :keys="item.lists" :valuelist="valuelist" :action-fix="'sfcsSMTSnView'"></v-form>
						</a-collapse-panel>
					</a-collapse>
				</a-spin>
				...
				<a-card style="width:100%" :tab-list="tabList" :active-tab-key="key" :headStyle="heads"
					...
					<v-table :sn="tablesn" :types="tabletype" :key="tablekey" :cols="tablelist" :order="tableorder"
						:action-fix="'sfcsSMTSnView'" :api="tableapi">
					</v-table>
				</a-card>
			</a-tab-pane>
		</a-tabs>
...
<script>
	...
	//const group = ['lot', 'palletno', 'weight']
	...
	export default {
		...
		methods: {
			...
			onTabChange(key, type) {
				...
				} else if (key == 'syssnlog') {
					this.tabletype = 'snlog'
				} else {
					this.tabletype = key
				}
			...
			handle_snlist(element) {
				...
				reqPost('SFCS_SMT', params_sn).then((res) => {
					element.map((e) => {
						res.data.some((a) => {
							if (e === a.sn) {
								arr.push(e)
			...
			handle_sn(element) {
				...
				// 	LINENAME: 'lineName',
				// 	LASTROUTECODENAME: 'lastRouteCodeName',
				// 	WIPROUTECODENAME: 'wipRouteCodeName',
				// 	STATUSNAME: 'statusName',
				// 	TIME: 'time',
				// 	ROUTENAME: 'routeName',
				// 	// IMEI: 'imei',
				// 	//HDIPN: 'hdiPn',
				// 	//  HDIVERSION: 'hdiVersion',
				// 	TARGETQTY: 'targetQty',
				// 	SOKEY: 'soKey',
				// 	OEMPN: 'oemPn',
				// 	SHIPTIME: 'shipTime',
				...
				// 	COA: 'coa',
				// 	// ASSYSN: 'assySn',
				// 	//  REPAIREDCOUNTER: 'repairedCounter',
				// 	// ODISSW: 'odisSw',
				// 	PRODUCTSN: 'productSn',
				// 	PALLETNO: 'palletNo',
				// 	SITENAME: 'siteName',
				// 	REVISIONNO: 'revisionNo',
				// 	TURNINORDERNO: 'turnInOrdNo',
				// 	LANCODE: 'lancode',
				// 	//   LOT: 'lot',
				// 	BATCHNO: 'batchNo',
				// 	// weight: 'weight',
				...
				reqPost('SFCS_SMT', params).then((res) => {
					...
					if (res.data.length > 0) {
						...
						this.handle_wo(res.data[0].wono)
					} else {
			...
			handle_bios(wo) {
				...
				reqPost('SFCS_G5', params).then((res) => {
					this.listLoading = false
